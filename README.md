...
4. **行动落地** — 输出 2-3 个可直接开拍的切入角度，每个角度附带
   **🧪 A/B 测试 Hook 矩阵**（冲突对立型 / 悬念好奇型 / 情绪共鸣型 3 套黄金 3 秒话术，含主打人群）、
   分镜提纲（标注 💬 评论区互动引导点）、执行步骤、变现路径与合规风险
5. **🛡️ 品牌合规与风控** — 广告法敏感词 / 极限词体检，输出风险等级（Low / Medium / High / Ban）、
   命中词清单与合规替换建议
...
- **🧪 A/B 测试 Hook 矩阵**：顶部用 [`render_hook_matrix()`](app.py) 以 `st.columns(3)`
  并排渲染 3 套风格 Hook 卡片（冲突对立型 / 悬念好奇型 / 情绪共鸣型），
...
> 决策建议、A/B Hook 矩阵、互动引导、风控评级与分镜表格会被格式化为标准 Markdown，可直接交给编导 / 摄影团队落地。
...
- **A/B 测试 Hook 矩阵**：[`ActionableInsight.golden_hooks`](hotspot_analysis/models.py:153) 以
  [`HookOption`](hotspot_analysis/models.py:138)（style / script / target_persona）承载 3 套风格话术，
  经 [`hook_by_style()`](hotspot_analysis/models.py:193) 按「冲突 / 悬念 / 共鸣」取用；
  旧字段 `hook`（单句 str）通过 `_migrate_legacy_hook` 校验器**自动向后兼容**迁移。
...
当前状态：**92 passed / 2 skipped**（跳过项为需要浏览器 / 可选依赖的用例）。
